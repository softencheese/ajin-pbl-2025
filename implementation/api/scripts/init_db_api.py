...
def create_lots(client: APIClient, data: dict):
    ...
    # 공정 정보 조회
    processes = client.get("/api/v1/processes", params={"per_page": 100})
    process_list = processes.get("items", [])
    
    receiving_process = next((p for p in process_list if p["process_code"] == "SHEARING"), None)
    press_process = next((p for p in process_list if p["process_code"] == "PRESS"), None)
    assembly_process = next((p for p in process_list if p["process_code"] == "ASSEMBLY"), None)
    ...
    for w in data["wip_items"]:
        ...
        if not press_process:
            print(f"  ⚠️  {item_code} - PRESS 공정을 찾을 수 없음")
            skipped_count += 1
            continue        
    ...
    for p in data["product_items"]:
        ...
        if not assembly_process:
            print(f"  ⚠️  {item_code} - ASSEMBLY 공정을 찾을 수 없음")
            skipped_count += 1
            continue
        
        # PRODUCT LOT 생성 (조립 공정)
        lot_data = {
            "item_id": item["id"],
            "quantity": 50,
            "production_date": today.isoformat(),
            "process_id": assembly_process["id"],
            "worker_name": "조립담당",
            "qc_passed": True,
