...
class Lot(BaseModel):
    ...
    item_id = Column(BigInteger, ForeignKey("items.id"), nullable=False, index=True, comment='품목 ID')
    quantity = Column(Integer, nullable=False, comment='현재 수량')
    initial_quantity = Column(Integer, nullable=False, comment='초기 수량 (목표)')
    produced_quantity = Column(Integer, nullable=False, default=0, comment='누적 생산 수량')
