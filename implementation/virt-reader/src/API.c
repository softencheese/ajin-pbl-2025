...
int Post_JSON_to_API(json_object *json, const char *url, int is_response_print)
{
	...
	if (json == NULL)
	{
		fprintf(stderr, "Post_JSON_to_API: json is NULL\n");
		return -1;
	}
	...
	if(curl) {
		...
		if (res == CURLE_OK && http_code >= 200 && http_code < 300) {
			if (chunk.data) {
				...
				if (parsed_json) {
					json_object *success_field;
					if (json_object_object_get_ex(parsed_json, "success", &success_field)) {
						if (json_object_get_boolean(success_field)) {
							ret = 0;
						} else {
							fprintf(stderr, "API returned success:false\n");
						}
					}
...
int Put_JSON_to_API(json_object *json, const char *url, int is_response_print)
{
	...
	if (json == NULL)
	{
		fprintf(stderr, "Put_JSON_to_API: json is NULL\n");
		return -1;
	}
