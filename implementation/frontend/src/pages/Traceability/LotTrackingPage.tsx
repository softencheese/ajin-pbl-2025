...
export function LotTrackingPage() {
    ...
    const columns: ColumnsType<LotTraceData> = [
        ...
        },
        {
            ...
            key: 'quantity',
            width: 140,
            render: (_: unknown, record: LotTraceData) => (
                <Space direction="vertical" size={2} style={{ width: '100%' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span style={{ fontSize: '0.85em', color: '#666' }}>현재수량:</span>
                        <Tag color={(record.quantity ?? 0) > 0 ? "green" : "default"} style={{ margin: 0 }}>{record.quantity ?? 0}</Tag>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span style={{ fontSize: '0.85em', color: '#666' }}>누적생산:</span>
                        <Tag color="blue" style={{ margin: 0 }}>{record.producedQuantity ?? 0}</Tag>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span style={{ fontSize: '0.85em', color: '#666' }}>목표수량:</span>
                        <Tag color="default" style={{ margin: 0 }}>{record.initialQuantity ?? 0}</Tag>
                    </div>
                </Space>
            ),
        },
                                    ...
                                    {detailModal.genealogy.parents.map((parent, idx) => (
                                        ...
                                        >
                                            ...
                                            <div style={{ fontSize: 12, color: '#6c757d', marginTop: 4 }}>
                                                공정: <Tag color="blue" size="small">{parent.process_name || '-'}</Tag> |
                                                유형: {getItemTypeLabel(parent.item_type as any)} |
                                                소비량: <Text strong>{parent.quantity_consumed ?? 0}</Text>
                                            </div>
                                        </Card>
                                    ))}
                                </div>
